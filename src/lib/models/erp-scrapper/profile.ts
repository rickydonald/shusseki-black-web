...
const URL =
    "https://erp.loyolacollege.edu/loyolaonline/students/report/studentProfile.jsp";

export async function scrapeStudentProfile<
    K extends readonly (keyof StudentProfile)[]
>(
    cookieHeader: string,
    fields?: K
): Promise<K extends undefined ? StudentProfile : Pick<StudentProfile, K[number]>>;

export async function scrapeStudentProfile(
    cookieHeader: string,
    fields?: readonly (keyof StudentProfile)[]
): Promise<StudentProfile | Partial<StudentProfile>> {
    ...
    ----------------------------------*/
    const BASE_URL =
        "//wsrv.nl/?url=https://erp.loyolacollege.edu/loyolaonline";
    ...
    const filtered: Partial<StudentProfile> = {};
    for (const key of fields) {
        if (key in profile) {
            (filtered as any)[key] = profile[key];
        }
    }
